...
[![npm (scoped)](https://img.shields.io/npm/v/%40xpack-dev-tools%2Fqemu-arm?label=xpm&color=blue)](https://www.npmjs.com/package/@xpack-dev-tools/qemu-arm/)
![GitHub Downloads (all assets, all releases)](https://img.shields.io/github/downloads/xpack-dev-tools/qemu-arm-xpack/total)
[![npm](https://img.shields.io/npm/dm/%40xpack-dev-tools%2Fqemu-arm?label=xpm%20downloads)](https://www.npmjs.com/package/@xpack-dev-tools/qemu-arm/)

The **xPack QEMU Arm** can be installed automatically, using the `xpm` command
(the recommended method), or manually, by downloading and unpacking one of the
portable archives.

## Automated install

The easiest (and recommended) way to install QEMU Arm
is by using the **binary xPack**, available
as <a href={`https://www.npmjs.com/package/@xpack-dev-tools/qemu-arm`}><code>@xpack-dev-tools/qemu-arm</code></a> from
...
<code>xpacks/@xpack-dev-tools/qemu-arm</code>.
...
**qemu-arm** package into the project:
...
* identify the platform specific archive for the latest available version
  of QEMU Arm, download it into a cache and unpack
...
* create a local symbolic link
  like <code>xpacks/@xpack-dev-tools/qemu-arm</code> pointing to the
...
* add links/forwarders into
  ...
  <code>xpacks/@xpack-dev-tools/qemu-arm/.content/bin</code>;
* add <code>@xpack-dev-tools/qemu-arm</code> to
  `package.json` as a
  development dependency; this associates a specific version of QEMU Arm with
...
it is also possible to install QEMU Arm only globally in
...
<CodeBlock language="sh"> {
`xpm install @xpack-dev-tools/qemu-arm@latest --global --verbose
`} </CodeBlock>
...
<CodeBlock language="sh"> {
`export PATH=$HOME/AppData/Roaming/xPacks/\@pack-dev-tools/qemu-arm/${customField('upstreamVersion')}-${customField('xpackSubversion')}.${customField('npmSubversion')}/.content/bin:$PATH
`} </CodeBlock>
...
<CodeBlock language="sh"> {
`export PATH=$HOME/Library/xPacks/\@pack-dev-tools/qemu-arm/${customField('upstreamVersion')}-${customField('xpackSubversion')}.${customField('npmSubversion')}/.content/bin:$PATH
`} </CodeBlock>
...
<CodeBlock language="sh"> {
`export PATH=$HOME/.local/xPacks/@xpack-dev-tools/qemu-arm/${customField('upstreamVersion')}-${customField('xpackSubversion')}.${customField('npmSubversion')}/.content/bin:$PATH
`} </CodeBlock>
...
<CodeBlock language="sh"> {
`xpm uninstall @xpack-dev-tools/qemu-arm --verbose
`} </CodeBlock>
...
the <a href={`https://github.com/xpack-dev-tools/qemu-arm-xpack/releases/`}>GitHub Releases</a> pages.
...
* <code>xpack-qemu-arm-{customField('upstreamVersion')}-{customField('xpackSubversion')}-win32-x64.zip</code>
...
To manually install the <b>xPack QEMU Arm</b>,
unpack the archive and move it to a location of your choice.

The recommended location is
the <code>%USERPROFILE%\\AppData\\Roaming\\xPacks\\qemu-arm</code> folder, for
example <code>C:\\Users\\ilg\\AppData\\Roaming\\xPacks\\qemu-arm\\xpack-qemu-arm-{customField('upstreamVersion')}-{customField('xpackSubversion')}</code>.
...
* <code>xpack-qemu-arm-{customField('upstreamVersion')}-{customField('xpackSubversion')}-darwin-x64.tar.gz</code>
* <code>xpack-qemu-arm-{customField('upstreamVersion')}-{customField('xpackSubversion')}-darwin-arm64.tar.gz</code>
...
The recommended location is
the <code>~/Library/xPacks/qemu-arm</code> folder, for
example <code>/Users/ilg/Library/xPacks/qemu-arm/xpack-qemu-arm-{customField('upstreamVersion')}-{customField('xpackSubversion')}</code>:

<CodeBlock language="console"> {
`mkdir -p ~/Library/xPacks/qemu-arm
cd ~/Library/xPacks/qemu-arm

tar xvf ~/Downloads/xpack-qemu-arm-${customField('upstreamVersion')}-${customField('xpackSubversion')}-darwin-x64.tar.gz
chmod -R -w xpack-qemu-arm-${customField('upstreamVersion')}-${customField('xpackSubversion')}
`} </CodeBlock>
...
* <code>xpack-qemu-arm-{customField('upstreamVersion')}-{customField('xpackSubversion')}-darwin-x64.tar.gz</code>
* <code>xpack-qemu-arm-{customField('upstreamVersion')}-{customField('xpackSubversion')}-darwin-arm64.tar.gz</code>
* <code>xpack-qemu-arm-{customField('upstreamVersion')}-{customField('xpackSubversion')}-darwin-arm.tar.gz</code>
...
The recommended location is
the <code>~/.local/xPacks/qemu-arm</code> folder, for
example <code>/home/ilg/.local/xPacks/qemu-arm/xpack-qemu-arm-{customField('upstreamVersion')}-{customField('xpackSubversion')}</code>:

<CodeBlock language="console"> {
`mkdir -p ~/.local/xPacks/qemu-arm
cd ~/.local/xPacks/qemu-arm

tar xvf ~/Downloads/xpack-qemu-arm-${customField('upstreamVersion')}-${customField('xpackSubversion')}-linux-x64.tar.gz
chmod -R -w xpack-qemu-arm-${customField('upstreamVersion')}-${customField('xpackSubversion')}
`} </CodeBlock>
